class Holiday < ActiveRecord::Base
	...
	def self.paydays (initdate, finaldate, paydate, person, licenciasRemuneradas, licenciasNoRemuneradas, licenciasAmbulatorias, licenciasARL, faltasInjustificadas, licenciasSoat)
		...
		if admissiondate.to_date<=finaldate
			...
			person.transcriptions.where(registerdate: initdate..finaldate).each do |t|	
				
				if diasFestivos.include?((t.registerdate.to_date).strftime("%Y-%m-%d"))
				  	if (t.registerdate.to_date).strftime("%A") == "Sunday"
				  		mensualidad[t.registerdate] = 1
				  	end
				else
				  	mensualidad[t.registerdate] = 1
			  	end
